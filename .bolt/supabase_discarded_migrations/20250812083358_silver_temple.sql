/*
  # Custom Services System for Consultants and Clients

  1. New Tables
    - `custom_services` - Consultant's custom services with pricing
    - `service_orders` - Client orders for custom services
    - `service_payments` - Payment tracking for services

  2. Security
    - Enable RLS on all tables
    - Consultants can manage their own services
    - Clients can view and order services
...
CREATE POLICY "Consultants can manage their services" ON custom_services
  FOR ALL TO authenticated
  USING (consultant_id = auth.uid())
  WITH CHECK (consultant_id = auth.uid());

CREATE POLICY "Clients can view active services" ON custom_services
  FOR SELECT TO authenticated
  USING (is_active = true);

CREATE POLICY "Admins can manage all services" ON custom_services
  FOR ALL TO authenticated
  USING (EXISTS (
...
CREATE POLICY "Clients can create orders" ON service_orders
  FOR INSERT TO authenticated
  WITH CHECK (client_id = auth.uid());

CREATE POLICY "Clients can view their orders" ON service_orders
  FOR SELECT TO authenticated
  USING (client_id = auth.uid());

CREATE POLICY "Consultants can view their orders" ON service_orders
  FOR SELECT TO authenticated
  USING (consultant_id = auth.uid());

CREATE POLICY "Consultants can update their orders" ON service_orders
  FOR UPDATE TO authenticated
  USING (consultant_id = auth.uid())
  WITH CHECK (consultant_id = auth.uid());

CREATE POLICY "Admins can manage all orders" ON service_orders
  FOR ALL TO authenticated
  USING (EXISTS (
    SELECT 1 FROM profiles 
    WHERE profiles.id = auth.uid() AND profiles.role = 'admin'
...
CREATE POLICY "Users can view their payments" ON service_payments
  FOR SELECT TO authenticated
  USING (EXISTS (
...
CREATE POLICY "System can manage payments" ON service_payments
  FOR ALL TO authenticated
  USING (true)
  WITH CHECK (true);
