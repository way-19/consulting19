...
const LoginPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('[AUTH] signIn error', { status: err?.status, message: err?.message, name: err?.name });
  ...
  const showProfileMissing = user && !authLoading && !profile;
            ...
            <div className="mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-sm text-yellow-800">
                  Giriş başarılı fakat profil bulunamadı. Hesabınıza rol atanmamış olabilir.
                </span>
              </div>
              <button
